...
| WIF | Candidate/live-proof already bind principal, project number, audience, and non-secret credential configuration hash. ADK and promotion now use the same strict secret-free shape: actual `gcloud` principal, project ID/number, external-account audience/impersonation target, and configuration hash must all match the expected federation before readback or traffic preflight. | Authenticated `gcloud` readback is required. | External target remains unobserved until its workflow emits a provider receipt. |
| ADK / Gemini | Canary tests require an exact source-bound challenge/response and keep provider-ready separate from `ADK_RUNTIME_OBSERVED`. The workflow derives its pre-canary health URL from the same provider readback (never its own not-yet-published output); a wrong-source, non-observed, malformed, or post-hash-mutated receipt fails before upload. | Live provider invocation only occurs through the guarded canary. | External provider remains unobserved until its independently produced receipt exists. |
...
| Cross-mission receipt binding | The generic Judge could accept a hash-valid receipt with the same payload hash from another manifest/revision. | Closed: receipt lookup now requires matching payload, manifest and revision; the negative cross-mission regression blocks it. |
| External receipts | ADK Canary receipt upload previously had no independent self-integrity check, and candidate/live-proof/promotion remain separate provider receipt chains. | ADK upload now recomputes and rejects a wrong-source, non-observed, malformed, or mutated receipt before GitHub artifact upload; it remains a scoped upload-integrity check, not canonical/provider truth. |
| WIF symmetry | ADK and promotion did not prove the generated external-account configuration against the active Google principal/project. | Closed statically: both fail before provider readback/preflight unless actual principal, project number, expected federation and secret-free configuration hash agree. A provider workflow run is still required for an external observation. |
